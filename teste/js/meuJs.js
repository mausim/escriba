...
jQuery(document).ready(function () {
    ...
    *
    *****************************************
    ATUALIZADO POR MAURO EM 18/10/2022 09:09 hs
    ...
    *
    - GERAÇÃO DE PDF - Criei vários botões de exibição de pdf
   ...
   // jQuery(".claFlexDash").hide();
    ...
    //window.open("https://script.google.com/macros/s/AKfycbwApc70Q-apzL4p_IJ69aJRo_7jaM-bnGzTny_Efth_P0NASjSSwcO1x1EvnyPe4aff/exec")
    //-
...
var xValues2 = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];
...
new Chart("myChart2", {
  ...
  data: {
    labels: xValues2,
    datasets: [{ 
      data: [0,0,0,0,0,100,130,220,430,0,0,0],
      borderColor: "#f35454",
      fill: false
    }, { 
      data: [1600,1700,1700,1900,2000,2700,4000,5000,3000,0,0,0],
      borderColor: "#0a0857",
      fill: false
    }]
  },
  options: {
    ...
    title:{
        display: true,
        text: "Comparativo por Meses"
    }
  }
});
*/
 //GRAFICOS TERMINA AQUI   
        ...
        if(idPrincipal*1>0 && vencimento != '' && idAprovador*1>0 && idFaturado*1>0 && idContato *1>0 && texto.length>0 && valor*1 > 0 ){
            ...
            contato_tabfaturados,aprovador_tabpessoas,descricao,valor,obs,data_insercao,status)
               ...
               let arrayValores = [idPrincipal,pastasAdicionais,vencimento,idRedator,idFaturado,idContato,idAprovador,
                ...
                //-------------------post----------------
                jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 4, sqlInsert: sqlInsert, tripa: tripa }, function (retorno) {
...
jQuery(document).on("click","#idBtnSistDespPesPesq",function (){
    ...
    if(idPasta * 1 > 0 && dataInicial != "" && dataFinal != ""){
        if(status*1>0 && nhrd*1>0){
            //quero um status e uma inclusão específicos
    ...
    END status,d.pasta_tabpastas,
        ...
        AND d.incluir = `+nhrd;
             ...
             if(status*1==0 && nhrd*1>0){
            //qualquer status, mas uma inclusão específica
            sql = `SELECT d.id,d.data,
            CASE 
            WHEN d.incluir = 1 THEN 'Escritório'
            WHEN d.incluir = 2 THEN 'NH'
        ...
        if(status*1==0 && nhrd*1==0){
             //qualquer status e qualquer inclusão
             sql = `SELECT d.id,d.data,
             CASE 
             WHEN d.incluir = 1 THEN 'Escritório'
             WHEN d.incluir = 2 THEN 'NH'
    ...
    if(idPasta * 1 > 0 && dataInicial == "" && dataFinal == ""){
        ...
        WHEN d.incluir = 3 THEN 'RD'
        END incluir,
        d.descricao,d.valor,d.data_insercao,d.status as intstatus,
        CASE 
        WHEN d.status = 1 THEN 'Rascunho'
        WHEN d.status = 2 THEN 'Pendente'
        WHEN d.status = 3 THEN 'Aprovada' 
    ...
    if(idCli * 1 > 0 && idPasta * 1 == 0 && dataInicial == "" && dataFinal == ""){
        sql = `SELECT d.id,d.data,
        CASE 
        WHEN d.incluir = 1 THEN 'Escritório'
        WHEN d.incluir = 2 THEN 'NH'
        WHEN d.incluir = 3 THEN 'RD' 
        END incluir,d.descricao,d.valor,d.data_insercao,d.status as intstatus,CASE 
        WHEN d.status = 1 THEN 'Rascunho'
        WHEN d.status = 2 THEN 'Pendente'
        WHEN d.status = 3 THEN 'Aprovada' 
        WHEN d.status = 7 THEN 'Cancelada/Reprovada' 
        END status,d.pasta_tabpastas,
        p1.pessoa as redator,p2.pessoa as aprovador,f.materia  
        FROM tabdespesas d 
        INNER JOIN tabpessoas p1 
...
jQuery(document).on("change","#idChkAlmoAdmPIA",function (){
 ...
 if(intIncluir2 *1==1){
    ...
    jQuery("#idBtnAlmoPesMoI").click(function () {
        ...
        let siglaNaMaquina = jQuery("#idSpaLogNaMaquinaSigla").text();
        let contador = 0;
        var arrayIdsAlmo = jQuery("input[class='claChkPed']:checkbox:checked").map(function () {
            return jQuery(this).val();
        }).get();
        let intEscolha = jQuery("input[name=namRadAlmoPesAlP]:checked").val();
        //alert (intEscolha+ " type: "+typeof intEscolha+" "+arrayIdsAlmo.length+" type: "+typeof arrayIdsAlmo);
        if (arrayIdsAlmo.length > 0 && intEscolha * 1 > 0) {
            let i = 0;
            for (i; i < arrayIdsAlmo.length; i++) {
                let sqlUpdate = `UPDATE tabalmo 
             set int_status=? 
             WHERE id = ?`;
                let arrayValores = [intEscolha, arrayIdsAlmo[i]];
                let intOperacao = 3;
                let tripa = fPrepara(arrayValores);
                fExecutaBD(intOperacao, sqlUpdate, tripa).then(function (retorno) {
    ...
    jQuery("#idBtnAlmoRegPoC").click(function () {
        ...
        objTab.empty();
        let concatena = `<tr><th>id</th><th>Solic</th><th>Material</th><th>Qtd</th><th>Apresentação</th><th>Sit</th></tr>`;
        //---
    ...
    jQuery(document).on("click", ".claCliTE", function () {
        ...
        WHERE e.id = `+ idTaxa;
        jQuery.post("/phpPaginas/bdExecuta.php", { sql: sql, intOperacao: 1 }, function (retorno) {
            //console.log(retorno);
            if (retorno.length > 20) {
                let obj = JSON.parse(retorno);
    ...
    jQuery("#idBtnSistNTESalvarA").click(function () {
        ...
        if ((idCli * 1 > 0 || idPas * 1 > 0) && idProf * 1 > 0 && valor * 1 > 0) {
            ...
            let tripa = fPrepara(arrayValores);
            //-------------------post----------------
            jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 3, sqlUpdate: sqlUpdate, tripa: tripa }, function (retorno) {
                ...
                if (retorno * 1 > 0) {
                    ...
                    if (intAtualizarTS == 1) {
                        ...
                        jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 3, sqlUpdate: sqlUpdateTE, tripa: tripaTE }, function (retornoTE) {
    ...
    jQuery("#idBtnPesSistUpFoto").click(function () {
       ...
       let minhaJanela = window.open("/teste/paginas/upload.html","upload","width=600,height=400");
    ...
    jQuery("#idBtnCronoRegAnexo").click(function () {
       ...
       let minhaJanela = window.open("/teste/paginas/uploadCrono.html","upload","width=600,height=400");
    }else{
    ...
    jQuery("#idBtnSistConsPesSalvar").click(function () {
        ...
        faixa_tabfaixas = ?,categoria_tabcategorias = ?,
         ...
         WHERE profissional_tabpessoas = ?`;
                        ...
                        jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 3, sqlUpdate: sqlUpdateT, tripa: tripaT }, function (retornoT) {
              ...
              AND int_status = 2`;
                                ...
                                jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 3, sqlUpdate: sqlUpdateTP, tripa: tripaTP }, function (retornoTP) {
             ...
             (?,?,?,?,?,?,?)`;
                                ...
                                jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 4, sqlInsert: sqlInsertTIP, tripa: tripaTIP }, function (retornoTIP) {
                                    if (retornoTIP * 1 > 0) {
                                        //inserção com sucesso
    ...
    SET data_edicao = now(),redator_tabpessoas = ?, cliente=?, 
        resp_tabpessoas=?, filial_tabfiliais=?, int_tipopessoa=?,
         cpfcnpj=?, email=?, telefone=?, celular=?, cep=?, 
        tipo_tabtiposlogradouro=?, logradouro=?, 
    ...
    WHERE id = ?`;
        ...
        let arrayValores = [intRedator, nomeRazao, intREsponsavel,
            intFilial, intTipoPessoa, cpfCnpj, email, telefone,
...
jQuery(document).on("blur","#idTxtExpeRegCEP",function (){
    ...
    */
    ...
    jQuery("#idBtnPesRegSalvar").click(function () {
        ...
        (?,?,?)`;
                    ...
                    jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 4, sqlInsert: sqlInsertT, tripa: tripaT }, function (retornoT) {
    ...
    FROM tabpastas`;
    try {
        jQuery.post("/phpPaginas/bdExecuta.php", { sql: sqlSel, intOperacao: 1 }, function (retorno) {
            if (retorno.length > 20) {
                let obj = JSON.parse(retorno);
        ...
        (redator_tabpessoas, cliente_tabclientes,pasta,
           materia, valor_hono,responsavel_tabpessoas, area_tabareas,
           documentacao,int_status)
           VALUES (?,?,?,?,?,?,?,?,?)`;
            //os dados abaixo têm de estar na mesma ordem das ?...
            //...da esquerda para a direita  na sentença de update
            let arrayValores = [intRedator, intCliente, novaPasta, materia,valorHono,
    ...
    jQuery("#idSelPasPesCli").change(function () {
       ...
       // jQuery("#idSelPasPesPas option[value='0']").prop('selected', true);
        //-------
        let sql = `SELECT c.cliente,p.id,p.pasta,p.materia,g.pessoa as responsavel,
 ...
 ORDER BY p.pasta desc`;
        ...
        fExecutaBD(intOperacao, sql, tripa).then(function (retorno) {
            if (retorno.length > 20) {
                ...
                for (i; i < obj.resultados.length; i++) {
                    ...
                    concatena =concatena + '<tr><th>id</th><th>Pasta</th><th>Matéria</th><th>Responsável</th><th>Área</th><th>Doc</th><th>Status</th></tr>';
                    concatenaPDF =concatenaPDF + '<tr><th>Pasta</th><th>Matéria</th><th>Responsável</th><th>Área</th><th>Doc</th><th>Status</th></tr>';
                    }//if i
    ...
    jQuery("#idSelPasPesPas").change(function () {
        ...
        objTab.empty();
        //-------
        let sql = `SELECT p.id,p.pasta,p.materia,g.pessoa as responsavel,
    ...
    ORDER BY p.pasta desc`;
        //-------
       let concatena = '<tr><th>id</th><th>Cliente</th><th>Pasta</th><th>Matéria</th><th>Responsável</th><th>Área</th><th>Doc</th><th>Status</th></tr>';
        let intOperacao = "1";
        let tripa = "";
        fExecutaBD(intOperacao, sql, tripa).then(function (retorno) {
            if (retorno.length > 20) {
...
window.open("/teste/paginas/expedicao.html","exped","width=600,height=400");
});//claBtnExpSta click
//------------------------------------------------------------
...
let minhaJanela = window.open("/teste/paginas/uploadHono.html","upload","width=600,height=400");
...
objTab.empty();
jQuery.post("/teste/phpPaginas/phpPush.php",{endEmail:email,pasEmail:senha,lidoNaoLido:lidoNaoLido},function (retorno){
objTab.append(retorno);
 });//post
...
<th>Prof</th><th>Uts</th><th>Taxa</th><th>Valor</th><th>Baixa</th><th>Data baixa</th><th>Sit</th></tr>`;
        }//if intBlocoOrigem==1
        if (intBlocoOrigem == 2) {
...
async function fFazUpdateBaixarReativar(intBaixReativ, arrayIds, justif, dataBaixa) {
    ...
    WHERE id = ?`;
        ...
        //------------------post
        await jQuery.post("/phpPaginas/bdExecuta.php", { intOperacao: 3, sqlUpdate: sqlUpdate, tripa: tripa }, function (retorno) {
            //  //console.log("Valor do retorno: "+retorno);
            if (retorno * 1 > 0) {
...
function fTraduzEnderecoEmMapa(endereco) {
    ...
    jQuery.getJSON("https://nominatim.openstreetmap.org/search/" + endereco + "?format=json&addressdetails=1&limit=1&polygon_svg=1", function (retorno) {
        if (retorno.length > 0) {
            ...
            fMarcaPontoMapa(latMedia, lonMedia, rua);
        } else {
            alert("Mapa não encontrado para este endereço. Melhore o endereço, até mesmo diminuindo a quantidade de informações");
        }
    });//getJSON
...
function fExecutaBD(intOperacao, sql, tripa) {
    ...
    let params = 'sql=' + sql + '&intOperacao=' + intOperacao + "&tripa=" + tripa;//se for intOperacao=1
    if (intOperacao == 3) params = 'sqlUpdate=' + sql + '&intOperacao=' + intOperacao + "&tripa=" + tripa;
    if (intOperacao == 4) params = 'sqlInsert=' + sql + '&intOperacao=' + intOperacao + "&tripa=" + tripa;
    //----------
    if (intOperacao < 5) {
        ...
        alert("Operação de BD não prevista na função fExecutaBD");
    }//if intOperacao<5
}//fExecutaBD
//---------------------------------------------------
...
objTab.empty();
let sql = `SELECT m.id, g.pessoa as redator,h.pessoa as solicitante,m.material,
...
sql = `SELECT m.id, g.pessoa as redator,h.pessoa as solicitante,m.material,
n.natureza, a.quantidade,a.data_insercao,a.int_status 
FROM tabalmo a 
INNER JOIN tabalmomat m 
ON a.material_tabalmomat = m.id 
...
function fPegaRascunhosPreencheTabela(intRedator,objTab){
    ...
    <th>Prof</th><th>Uts</th><th>Taxa</th><th>Valor</th><th>Baixa</th><th>Data baixa</th><th>Sit</th><th></th></tr>`;
