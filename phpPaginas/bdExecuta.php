...
require("../phpFuncoes/funcoes.php");
//*****
...
$meuArquivo=fopen("../vigia/selecao.txt","w");
...
case 3:
    //faz update
require("../phpClasses/classesPhp.php");
$meuArquivo=fopen("../vigia/update.txt","w");
$conexao = new ConexaoBD();
$minhaConn = $conexao->getConexao();
$minhaConn->exec('SET NAMES utf8');
...
require("../phpClasses/classesPhp.php");
$meuArquivo=fopen("../vigia/insercao.txt","w");
$conexao = new ConexaoBD();
$minhaConn = $conexao->getConexao();
$minhaConn->exec('SET NAMES utf8');
...
break;
case 10:
//se o login form admin, abre outro banco de dados (o escribaoffice01)    
require("../phpClasses/phpClasseAdmin.php");
$conexaoA = new ConexaoBDA();
$minhaConnA = $conexaoA->getConexaoA();
$minhaConnA->exec('SET NAMES utf8');
$sqlPassado = $_POST['sql'];
$meuArquivo=fopen("../vigia/selecao.txt","w");
fwrite($meuArquivo,$sqlPassado."\r\n");
fclose($meuArquivo);
...
//...campos e seus valores correspondentes
//https://www.sitepoint.com/community/t/get-pdo-column-name-the-easy-way/4031/16
$stA = $minhaConnA->prepare($sqlPassado);
$stA -> execute();
$raw_column_data = $stA->fetchAll(PDO::FETCH_ASSOC);
echo fStrDevolveJson($raw_column_data);
$minhaConnA = null;
break;
case 11:
        //este é um select PARA O CASO DA PESSOA NA MÁQUINA SER UM ADMINISTRADOR
        require("../phpClasses/phpClasseAdmin.php");
        $conexaoA = new ConexaoBDA();
        $minhaConnA = $conexaoA->getConexaoA();
        $minhaConnA->exec('SET NAMES utf8');
    ...
    break;
case 41:
        //Faz uma nova inserção PARA O CASO EM QUE O ADMINISTRADOR É QUE ESTÁ NA MÁQUIN A
        require("../phpClasses/phpClasseAdmin.php");
        $conexaoA = new ConexaoBDA();
        $minhaConnA = $conexaoA->getConexaoA();
        $minhaConnA->exec('SET NAMES utf8');
        ...
        fwrite($meuArquivo,"o que vai como parâmetro para inserção: ".$sqlInsertPassado." -> ".$tripa."\r\n");
        $meuArray=preg_split("/#@#/", $tripa);
        $stmtA = $minhaConnA->prepare($sqlInsertPassado);
        for($i=0;$i < count($meuArray);$i++){
        $valorVez=$meuArray[$i];
        fwrite($meuArquivo,$valorVez."\r\n");
        $stmtA -> bindValue($i+1, $valorVez);
        }//for i
        $stmtA->execute();
        $idConseguido = $minhaConnA->lastInsertId();//pega o id do registro recém-criado
        echo  $idConseguido;
        fclose($meuArquivo);
        break;  
case 31:
            //faz update PARA O CASO DO ADMINISTRADOR ESTAR NA MÁQUINA
            require("../phpClasses/phpClasseAdmin.php");
            $conexaoA = new ConexaoBDA();
            $minhaConnA = $conexaoA->getConexaoA();
            $minhaConnA->exec('SET NAMES utf8');
        ...
        $meuArray=preg_split("/#@#/", $tripa);
        fwrite($meuArquivo,"Quantidade de elementos no array: ".count($meuArray)." na sql: ".$sqlUpdatePassado."\r\n");
        $stmtA = $minhaConnA->prepare($sqlUpdatePassado);
        for($i=0;$i < count($meuArray);$i++){
        $valorVez=$meuArray[$i];
        fwrite($meuArquivo,"Valor da vez: ".$valorVez."\r\n");
        //fwrite($meuArquivo,"o que vai como parâmetro para update: ".$segundo."\r\n");
